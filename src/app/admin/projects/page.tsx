      ...
      <main className="min-h-screen bg-background text-foreground">
        ...
        <div className="k-container py-12 md:py-16">
          ...
          <section className="rounded-xl border border-error/30 bg-error/5 p-8">
            {" "}
            <p className="k-eyebrow mb-3 text-error"> Projects </p> <h1 className="k-heading-2 mb-3"> Unable to load projects </h1> <p className="k-body text-muted-foreground"> Something went wrong while loading the project list. Please try again. </p>{" "}
          </section>{" "}
        </div>{" "}
      </main>
    );
      ...
      <div className="k-container py-8 md:py-12">
        ...
        <header className="mb-10">
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            <div>
              {" "}
              <p className="k-eyebrow mb-3"> Admin / Projects </p> <h1 className="k-heading-1">Projects</h1> <p className="k-body-large mt-3 max-w-2xl text-muted-foreground"> Manage the projects displayed across the KURESHTIC website. </p>{" "}
            </div>{" "}
        ...
        {projects.length === 0 ? (
          <section className="rounded-xl border border-border bg-surface p-8 md:p-12">
            {" "}
            <p className="k-eyebrow mb-3"> No projects </p> <h2 className="k-heading-2 mb-3"> Your project library is empty. </h2> <p className="k-body mb-6 max-w-xl text-muted-foreground"> Create your first project to start building the KURESHTIC portfolio. </p>{" "}
                  ...
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mb-2 flex flex-wrap items-center gap-2">
                        {" "}
                        <h2 className="truncate text-base font-semibold"> {project.title} </h2> <span className="rounded-full border border-border px-2.5 py-1 text-xs text-muted-foreground"> {project.category} </span>{" "}
                      </div>{" "}
                      <p className="mb-3 text-sm text-muted-foreground"> {project.short_description} </p>{" "}
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        {" "}
                        <span className={`rounded-full px-2.5 py-1 ${project.published ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}> {project.published ? "Published" : "Draft"} </span> {project.featured && <span className="rounded-full bg-accent/10 px-2.5 py-1 text-accent"> Featured </span>} <span className="text-muted-foreground"> /{project.slug} </span>{" "}
                      </div>{" "}
                    </div>{" "}
